...
export function StellarProofCard({ vault, onCreateProof, isLoading }: StellarProofCardProps) {
  ...
  const status = hasReceipt ? (isNetworkVerified ? 'Stellar transaction confirmed' : 'Wallet receipt saved') : getStellarStatusLabel(vault.stellarStatus);
  ...
  return (
    <Card>
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <p className="text-sm font-black text-orange-700">Vault receipt</p>
          <h2 className="mt-2 text-2xl font-black text-slate-950">{status}</h2>
          <p className="mt-2 text-sm leading-6 text-slate-600">
            Save a receipt for this vault. It keeps your wallet address, plan details, and verification status in one place.
          </p>
        </div>
        {hasReceipt ? (
